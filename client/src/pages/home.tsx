            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
              <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-3">
                <Button 
                  ...
                  className="bg-gradient-to-r from-red-500 to-orange-500 hover:from-red-600 hover:to-orange-600 text-white px-3 sm:px-4 py-2 h-8 sm:h-9 text-sm"
                >
                ...
                <Select value={sortBy} onValueChange={(value: 'newest' | 'oldest' | 'popular' | 'controversial') => setSortBy(value)}>
                  <SelectTrigger className="w-full sm:w-32 h-8 bg-transparent border-none hover:bg-accent/30 text-sm">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
          ...
          <div className="glass border border-border/50 rounded-xl p-3 mb-3">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0 text-sm">
              <div className="grid grid-cols-3 sm:flex sm:items-center gap-3 sm:gap-4">
                <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-1.5 text-center sm:text-left">
                  <span className="text-lg sm:text-xl font-bold gradient-text">{stats.totalPosts}</span>
                  <span className="text-muted-foreground text-xs">posts</span>
                </div>
                <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-1.5 text-center sm:text-left">
                  <span className="text-lg sm:text-xl font-bold gradient-text">{stats.postsToday}</span>
                  <span className="text-muted-foreground text-xs">today</span>
                </div>
                <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-1.5 text-center sm:text-left">
                  <span className="text-lg sm:text-xl font-bold gradient-text">{stats.activeUsers}</span>
                  <span className="text-muted-foreground text-xs">users</span>
                </div>
              </div>
              <div className="text-xs text-muted-foreground bg-accent/30 px-2 py-1 rounded-full text-center sm:text-left">
                Platform Stats
              </div>
            </div>
          </div>
        ...
        {isLoading ? (
          ...
          <Card className="glass p-6 sm:p-8 text-center border-border/50">
            <div className="text-muted-foreground text-base">
              {searchQuery ? (
                <div className="space-y-2">
                  <i className="fas fa-search text-3xl text-primary/50 mb-3"></i>
                  <div>No results found for "{searchQuery}"</div>
                  <div className="text-sm">Try different keywords or check your spelling</div>
                </div>
              ) : (
                <div className="space-y-3">
                  <i className="fas fa-comments text-4xl text-primary/30 mb-3"></i>
                  <div className="text-xl font-bold gradient-text">Ready for Some Brutal Honesty?</div>
                  <div className="text-sm">Be the first to share your savage take</div>
                  <Button 
                    onClick={() => setPostModalOpen(true)}
